...
#define PRINT_DELAY 100 // 대화창에 관한 정의
#define LINE_LEN 100
...
void print_all(int Player_x, int Player_y, int Enemy_x, int Enemy_y);
...
int game_play()
{
	...
	print_all(Player_x, Player_y, Enemy_x, Enemy_y);
	...
	if (!tutorial)
	{
		...
		for (i = 0; i < 33; i++)
		{
			check_deg(&bef_key, LEFT, &deg_key);
			Player_x -= 2;
			print_character(deg_key, Player_x, Player_y, LEFT);
			Sleep(70);
		}

		Player_x -= 2;
		print_character(0, Player_x, Player_y, LEFT);
	...
	while (1)
	{
		...
		if (abs(Enemy_x - Player_x) < 7 && abs(Enemy_y - Player_y) < 4)
		{
			...
			while (Enemy_x != 146 || Enemy_y != 8)
			{
				...
				print_character(0, Player_x, Player_y, RIGHT);
		...
		if (UP <= key && key <= DOWN)
		{
			if (key == UP)
			{
				...
				{
					if (check_obs(Player_x, Player_y) == 3)
					{
						chase_sw = 1;
						amount += 300;
					}
					Player_y++;
					if (check_obs(Player_x, Player_y) == 2)					
						meet_NPC(Player_x, Player_y, Enemy_x, Enemy_y, &tutorial, amount);											
				}
			}
...
void print_str(char *name, char *str, int Player_x, int Player_y, int Enemy_x, int Enemy_y, int sel)
{
	...
	print_at(51, 36, name);
	print_at(132, 37, "TO SKIP PRESS 'ANY KEY'");
	gotoxy(80, 33);
	for (i = 0; str[i]; i++) {
		key = get_key();
		if (str[i] == '/')
...
int check_obs(int x, int y)
{
	if (146 < x || x < 0 || 37 < y || y < 0)					return 1; //벽의 영역
...
void meet_NPC(int Player_x, int Player_y, int Enemy_x, int Enemy_y, int *tutorial, int amount)
{
	...
	if (!(*tutorial))
	{
		...
		print_str("진빈", "그래.../그게 가장 큰 문제지.../게다가 예지가 아까 부터 308강의실에서 서성이고 있어/앞에 있는 문으로 잠시 나가면 주의를 돌릴 수 있을거야", Player_x, Player_y, Enemy_x, Enemy_y, 2);
		...
		print_str("진빈", "아!! 맞다 그리고 아까 도윤이형이 흘리고 간 동전지갑이/강의실 앞에 있어 보다 빨리 돈을 모을 수 있을거야/하지만 예지가 지갑의 존재를 알고 있기 때문에/널 바로 쫓아올거야!", Player_x, Player_y, Enemy_x, Enemy_y, 2);
		print_str("현우", "형 고마워! 바로 시작해 볼게!!", Player_x, Player_y, Enemy_x, Enemy_y, 0);
		(*tutorial) = 1;
	}
